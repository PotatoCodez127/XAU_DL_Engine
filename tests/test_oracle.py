...
from models.oracle_lstm import train_oracle, TradingSequenceDataset
...
def test_dataset_scaler_export(mock_labeled_data, tmp_path):
    ...
    dataset = TradingSequenceDataset(mock_labeled_data, window_size=5, scaler_path=str(scaler_path))
    
    assert os.path.exists(scaler_path), "Dataset failed to export the Z-score scaler."
    
    scaler_data = np.load(scaler_path)
    assert 'mean' in scaler_data and 'std' in scaler_data, "Scaler missing mean or std arrays."
    assert len(scaler_data['mean']) == 2, "Scaler should only track the 2 non-env stationary features."
...
def test_oracle_training_loop(mock_labeled_data, tmp_path):
    ...
    try:
        ...
        assert os.path.exists(model_path), "Training loop completed but failed to save .pth weights."
