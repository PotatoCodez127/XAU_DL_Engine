...
def test_environment_mechanics():
    ...
    for i in range(1000): # Test over 1000 candles
        ...
        if previous_position == 0 and env.position != 0:
            print(f"Step {info['step']}: 🟢 ENTERED {'LONG' if env.position == 1 else 'SHORT'} | Entry: {env.entry_price:.2f} | SL: {env.sl_price:.2f} | TP: {env.tp_price:.2f}")
            trades_executed += 1
            
        # Detect Trade Exit
        elif previous_position != 0 and env.position == 0:
            print(f"Step {info['step']}: 🔴 EXITED TRADE | Reward Triggered: {reward:.4f} | Bars Held: {env.bars_held}\n")
