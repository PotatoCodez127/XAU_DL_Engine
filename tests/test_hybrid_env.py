...
def test_environment_scaler_persistence(mock_feature_data, tmp_path):
    ...
    env1 = HybridTradingEnv(mock_feature_data, window_size=5, scaler_path=str(scaler_file))
    ...
    assert len(env1.feature_std) == 1, "Should only track 1 non-env feature (dist_ema_50)"
    ...
    env2 = HybridTradingEnv(mock_feature_data, window_size=5, scaler_path=str(scaler_file))
    
    # Assert that env2 loaded the original std dev, rather than recalculating it on the modified test data
...
def test_atr_lookahead_bias(mock_feature_data, tmp_path):
    scaler_file = tmp_path / "mock_scaler.npz"
    env = HybridTradingEnv(mock_feature_data, window_size=5, scaler_path=str(scaler_file))
    
    atr_values = env.df['env_atr'].values
    
    # The first 14 rows should have uniform ATR due to the strict backfill limit
    assert atr_values[0] == atr_values[13], "Initial ATR window was not correctly restricted."
    
    # Rows after the 14th should be dynamic and differ from the backfilled block
    assert atr_values[0] != atr_values[25], "ATR calculation is statically leaking future global volatility."